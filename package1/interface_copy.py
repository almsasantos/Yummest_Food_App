...
class App(Frame):
    ...
    def cheapest_restaurant(self, total_choices_with_drink_and_starter):
        ...
        if starter_guay != ['']:
            ...
            self.df_starter_choice = self.df_starter.loc[self.df_starter['product'].str.contains(starter_guay[0]), :]
            self.prices_df[f'price_{starter_guay[0]}'] = self.df_starter_choice.groupby('restaurant')['price'].min()
