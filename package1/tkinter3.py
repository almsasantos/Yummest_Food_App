...
class App(Frame):
    def __init__(self, master):
        ...
        self.starter_button = Button(self.starter_frame, text='Enter', command=self.starter_choice)
        ...
        Radiobutton(self.drink_frame, text='Champán', variable=self.drink_choice, value='champán', bg='white').grid(row=14, column=10, sticky=W)
        ...
        self.map.grid(row=51, column=2, columnspan=5)
        ...
        #Exit button
        self.button_quit = Button(self.master, text='Exit', command=master.quit)
        self.button_quit.grid(row=53, column=5)
    ...
    def predict_foods(self, model_file, type_list_choices, list_of_images):
        ...
        for img in list_of_images:
            ...
            #self.__dict__.food_choices.append(self.pred_value)
    ...
    def cheapest_restaurant(self, total_choices_with_drink_and_starter):
        ...
        global food_price
        self.df_initial = pd.read_csv('../restaurant_dataframe.csv')
        ...
        if starter_guay != ['']:
            df_starter = self.df_initial.loc[self.df_initial['type'].str.contains('entrante'), :]
            df_starter_choice = df_starter.loc[df_starter['product'].str.contains(starter_guay[0]), :]
            self.prices_df[f'price_{starter_guay[0]}'] = df_starter_choice.groupby('restaurant')['price'].min()
            ...
            if 0 not in list(self.prices_df_merge.isnull().sum(axis=1)):
                self.prices_df_merge.dropna(thresh=len(total_choices_with_drink_and_starter)-1, axis=0, inplace=True)
    ...
    def defining_food_choices(self):
        global starter_guay
        global drink_guay
        starter_guay = self.get_starter_choice()
        if self.get_plate_choice() != ['']:
            ...
            self.predict_foods(model_file='/home/almsasantos/Desktop/Ironhack/Final-Project/models/best_model_45class_salads.hdf5', type_list_choices=plates, list_of_images=self.get_plate_choice())
        ...
        if self.get_desert_choice() != ['']:
            ...
            self.predict_foods(model_file='/home/almsasantos/Desktop/Ironhack/Final-Project/models/best_model_9desert.hdf5', type_list_choices=deserts, list_of_images=self.get_desert_choice())
