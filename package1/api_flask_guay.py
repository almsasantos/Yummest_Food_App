...
def rest_price():
    ...
    food_dict = food_price
    if cheapest_classification == 'none':
        return render_template('cheapest_rest_guay.html', data=cheapest_words, food_dict=food_dict, not_available=not_available)
    else:
        return render_template('cheapest_rest_guay_classification.html', data=cheapest_words, food_dict=food_dict, not_available=not_available)
...
def venue_suggestion():
    ...
    if len(df_venue_options) > 2:
        two_options_venue = f"Nearby you can visit a {df_venue_options['categories'][0].upper()} called {df_venue_options['name'][0]} \nand a {df_venue_options['categories'][1].upper()} called {df_venue_options['name'][1]}!"
